...
namespace ProjMVC.Controllers
{
    public class HomeController : Controller
    {
       ...
       public static  string  token;
        ...
        public async Task<ActionResult> Contact(string search, int? id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                {
                    //     result = responseData.FindAll(u => u.Name.Contains(search)).ToList();
                    result = responseData.FindAll(p => p.Id == id).ToList();
                }
                else if (search != null && (id == 0 || id == null))
                {
                    result = responseData.FindAll(u => u.Name.Contains(search)).ToList();
                ...
                {
                    result = responseData.FindAll(p => p.Id == id && p.Name.Contains(search)).ToList();
                }
